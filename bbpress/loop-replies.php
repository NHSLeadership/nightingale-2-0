...
<div id="topic-<?php bbp_topic_id(); ?>-replies" class="bs-item-list bs-forums-items bs-single-forum-list bb-single-reply-list list-view">

	<?php
	if ( ! empty( bbp_get_topic_id() ) ) {
		?>
        <div class="bs-item-wrap bs-header-item align-items-center no-hover-effect">
            ...
            <div class="item flex-1">
                        ...
                        <div class="bb-topic-states push-right">
                        	...
                        	<?php if ( !empty( bbp_get_topic_close_link() ) ) { ?>
								<?php if ( bbp_is_topic_open() ) { ?>
	                                <span data-balloon-pos="up" data-balloon="<?php _e( 'Close', 'nightingale' ); ?>"><i
	                                            class="bb-topic-status open"><?php echo bbp_get_topic_close_link(); ?></i></span>
								<?php } else { ?>
	                                <span data-balloon-pos="up" data-balloon="<?php _e( 'Open', 'nightingale' ); ?>"><i
							...
							if ( ! bbp_is_topic_super_sticky( bbp_get_topic_id() ) && !empty( bbp_get_topic_stick_link() ) ) {
                                ...
                                <span data-balloon-pos="up" data-balloon="<?php _e( 'Unstick', 'nightingale' ); ?>">
                                    <i class="bb-topic-status bb-sticky sticky"><?php echo bbp_get_topic_stick_link(); ?></i>
                                    </span><?php
								} else { ?>
                                <span data-balloon-pos="up" data-balloon="<?php _e( 'Sticky', 'nightingale' ); ?>">
							...
							if ( !empty( bbp_get_topic_stick_link() ) ) {
	                            ...
	                            <span data-balloon-pos="up" data-balloon="<?php _e( 'Unstick', 'nightingale' ); ?>"><i
	                            ...
	                            <span data-balloon-pos="up"
	                                  data-balloon="<?php _e( 'Super Sticky', 'nightingale' ); ?>"><i
                            ...
                            <span class="bb-favorite-wrap" data-balloon-pos="up"
                                  data-balloon="<?php _e( 'Unfavorite', 'nightingale' ); ?>"
                                  data-unfav="<?php _e( 'Unfavorite', 'nightingale' ); ?>"
                                  data-fav="<?php _e( 'Favorite', 'nightingale' ); ?>"><i
                                        class="bb-topic-status bb-favorite-status favorited"><?php bbp_user_favorites_link(); ?></i>
                                </span><?php
							} else { ?>
                            <span class="bb-favorite-wrap" data-balloon-pos="up"
                                  data-balloon="<?php _e( 'Favorite', 'nightingale' ); ?>"
                                  data-unfav="<?php _e( 'Unfavorite', 'nightingale' ); ?>"
                                  data-fav="<?php _e( 'Favorite', 'nightingale' ); ?>"><i
				...
				<span class="bs-replied">
					<span class="bbp-topic-freshness-author"><?php bbp_author_link( array( 'post_id' => bbp_get_topic_last_active_id(), 'size' => 1 ) ); ?></span> <?php _e( 'updated', 'nightingale' ); ?> <?php bbp_topic_freshness_link(); ?>
				</span>
					...
					<span class="bs-replies"><?php
						if ( bbp_show_lead_topic() ) {
							...
							$topic_reply_text = $topic_post_count > 1 ? __( 'Posts', 'nightingale' ) : __( 'Post', 'nightingale' );
						}
						?>
    ...
    <div id="comments" class="nhsuk-list-panel comments-area">
        <ul class="nhsuk-list-panel__list comment-list">
            ...
            if ( bbp_thread_replies() ) : ?>
                <?php nightingale_bbp_list_replies(); ?>
            <?php else : ?>
