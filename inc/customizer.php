...
function nightingale_add_blog_settings( $wp_customize ) {
	...
	if ( in_array( 'buddyboss-platform/bp-loader.php', $active_plugins, true ) ) {
		...
		$wp_customize->add_section(
			'section_buddyboss',
			array(
				'title'       => esc_html__( 'BuddyBoss Theme Options', 'nightingale' ),
				'description' => esc_attr__( 'Customise your BuddyBoss Display settings', 'nightingale' ),
				'priority'    => 70,
			)
		);
		...
		$wp_customize->add_setting(
			...
			array(
				'default'           => get_theme_file_uri('assets/images/svg/group-default.svg'),
				'sanitize_callback' => 'nightingale_sanitize_select',
			)
		);
		$wp_customize->add_control(
			new WP_Customize_Media_Control(
				...
				'buddyboss_group_cover_default',
				array(
					'settings'    => 'buddyboss_group_cover_default',
					'mime_type'   => 'image',
					'section'     => 'section_buddyboss',
					'label'       => esc_html__( 'Default Group Image', 'nightingale' ),
					'description' => esc_html__( 'Select a default image to use if no group image uploaded.', 'nightingale' ),
				)
			)
		);
	} // end BuddyPress Theming options
