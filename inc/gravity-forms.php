...
function nightingale_clean_gf_inputs( $input, $field, $value, $lead_id, $form_id )
{
	...
	switch ($field->type) { // available values: radio, poll, survey, email, number, multiselect, text, textarea, checkbox, select
		...
		case 'poll':
			$choices.= '<pre>';
			$choices.= print_r($field);
			$choices.= '</pre>';
			break;
		case 'radio':
			$wrapper .= '<div class="nhsuk-radios  nhsuk-radios" id="input_' . $form_id . '_' . $field->id . '">';
			$i = 0;
			foreach ($field->choices as $selection) {
...
add_filter( 'gform_get_form_filter', function ( $form_string, $form ) {
	...
	//$form_string = str_replace( "gform_next_button", "gform_next_button nhsuk-button nhsuk-button__reverse", $form_string );
	//$form_string = str_replace( "gform_prev_button", "nhsuk-button nhsuk-button__secondary", $form_string );
	return $form_string;
}, 10, 2 );
