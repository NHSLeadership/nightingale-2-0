...
function nightingale_category_parents( $id, $link = false, $separator = '', $nicename = false, $visited = array(), $iscrumb = false ) {
	...
	if ( $parent->parent && ( $parent->parent !== $parent->term_id ) && ! in_array( $parent->parent, $visited, true ) ) {
		...
		if ( $link ) {
			$chain .= '<li class="nhsuk-breadcrumb__list-item">' . get_category_parents( $parent->parent, $link, $separator, $nicename ) . '</li>' . $separator;
		} else {
			$chain .= '<li class="nhsuk-breadcrumb__list-item"><a itemprop="item" href="' . get_category_parents( $parent->parent, $link, $separator, $nicename ) . '">' . $name . '</a></li>' . $separator;
		}
	}
	if ( $iscrumb ) {
		$chain .= '<li class="nhsuk-breadcrumb__list-item"><a itemprop="item" href="' . esc_url( get_category_link( $parent->term_id ) ) . '">' . $name . '</a></li>' . $separator;
	} elseif ( $link && ! $iscrumb ) {
		$chain .= '<li class="nhsuk-breadcrumb__list-item"><a itemprop="item" href="' . esc_url( get_category_link( $parent->term_id ) ) . '">' . $name . '</a>' . $separator . '</li>';
	} else {
		$chain .= '<li class="nhsuk-breadcrumb__list-item">' . $name . $separator . '</li>';
	}
...
function nightingale_breadcrumb() {
	...
	if ( true === nightingale_uncanny_breadcrumb_check() ) {
		...
		$breadcrumbs                    = sprintf(
			'<ol class="nhsuk-breadcrumb__list"><li class="nhsuk-breadcrumb__list-item"><a class="nhsuk-breadcrumb__link" href="%2$s">%3$s</a></li>%1$s</ol>',
			$trail,
			esc_url( home_url() ),
	...
	if ( true === nightingale_uncanny_breadcrumb_check() ) {
		...
		$breadcrumbs = str_replace( '<span itemscope', '<span class="nhsuk-breadcrumb__list-item" itemscope', $breadcrumbs );
		...
		printf(
			'<div class="nhsuk-width-container">
						<nav class="nhsuk-breadcrumb" aria-label="Breadcrumb">%1$s 
							<a class="nhsuk-back-link" href="%2$s"><span class="nhsuk-u-visually-hidden"> %3$s &nbsp;</span> %4$s</a>
						</nav>
					</div>',
...
function nightingale_breadcrumb_trail() {
	...
	if ( is_category() ) {
		...
		$trail .= '<li class="nhsuk-breadcrumb__list-item"><a  href="' . esc_url( get_category_link( $this_cat ) ) . '">' . esc_html( single_cat_title( '', false ) ) . '</a></li>';
	} elseif ( is_post_type_archive( 'tribe_events' ) ) {
		$trail = '<li class="nhsuk-breadcrumb__list-item">' . esc_html( tribe_get_event_label_plural() ) . '</li>';
	} elseif ( is_archive() && ! is_category() ) {
		$trail = '<li class="nhsuk-breadcrumb__list-item">' . esc_html( 'Archives', 'nightingale' ) . '</li>';
	} elseif ( is_search() ) {
		$trail = '<li class="nhsuk-breadcrumb__list-item">' . esc_html( 'Search Results', 'nightingale' ) . '</li>';
	} elseif ( is_404() ) {
		$trail = '<li class="nhsuk-breadcrumb__list-item">' . esc_html( '404 Not Found', 'nightingale' ) . '</li>';
		...
		$trail = '<li class="nhsuk-breadcrumb__list-item">' . the_title() . '</li>';
	} elseif ( is_singular( 'tribe_events' ) ) {
		$post   = $wp_query->get_queried_object();
		$trail  = '<li class="nhsuk-breadcrumb__list-item"><a href="' . esc_url( tribe_get_events_link() ) . '">' . esc_html( tribe_get_event_label_plural() ) . '</a></li>';
		$trail .= '<li class="nhsuk-breadcrumb__list-item event">' . esc_html( get_the_title( $post ) ) . '</li>';
		...
		if ( 0 !== $post->post_parent ) {
			...
			foreach ( $ancestors as $ancestor ) {
				if ( ( end( $ancestors ) !== $ancestor ) && ( ( $home_page !== $ancestor ) ) ) {
					...
					$trail         .= '<li class="nhsuk-breadcrumb__list-item"> <a href="' . esc_url( get_permalink( $ancestor ) ) . '">' . esc_html( wp_strip_all_tags( apply_filters( 'single_post_title', get_the_title( $ancestor ) ) ) ) . '</a></li>';
	...
	if ( ! ( is_archive() || is_category() || is_post_type_archive() || is_search() || is_404() || is_singular( 'tribe_events' ) ) ) {
		$trail .= '<li class="nhsuk-breadcrumb__list-item current">' . esc_html( get_the_title() ) . '</li>';
	}
