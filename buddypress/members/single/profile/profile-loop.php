...
<header class="entry-header profile-loop-header profile-header flex align-items-center">
	<h1 class="entry-title bb-profile-title"><?php esc_attr_e( 'View Profile', 'nightingale'); ?></h1>

	<?php if( bp_is_my_profile() ) { ?>
		<a href="<?php echo $edit_profile_link; ?>" class="push-right button outline small"><?php esc_attr_e( 'Edit Profile', 'nightingale'); ?></a>
	<?php } ?>
</header>
...
<?php else: ?>
	...
	<div class="info bp-feedback">
		...
		<p>
			<?php
			if ( bp_is_my_profile() ) {
				esc_html_e( 'You have not yet added details to your profile.', 'nightingale');
			} else {
				esc_html_e( 'This member has not yet added details to their profile.', 'nightingale');
			}
			?>
		</p>
	</div>
