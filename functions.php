...
/**
 * Filters the script HTML tag to add `type="module"` for specific scripts.
 *
 * WordPress does not natively support setting `type="module"` via wp_enqueue_script.
 ...
 * @return string Modified script tag with `type="module"` if handle matches.
 */
function add_type_module_to_specific_script( $tag, $handle, $src ) {
	if ( 'nightingale-nhs-library' === $handle || 'nightingale-nhs-library-init' === $handle ) {
		// This script is properly enqueued via wp_enqueue_script().
		// We override the tag to add type="module".
		// phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript
		return '<script type="module" src="' . esc_url( $src ) . '"></script>';
	}
	return $tag;
}
add_filter( 'script_loader_tag', 'add_type_module_to_specific_script', 10, 3 );

function add_nhsuk_frontend_body_class_script() {
    echo "<script>document.body.className += ' js-enabled' + ('noModule' in HTMLScriptElement.prototype ? ' nhsuk-frontend-supported' : '');</script>";
}
add_action( 'wp_body_open', 'add_nhsuk_frontend_body_class_script' );
...
add_filter( 'get_custom_logo', 'inject_logo_markup_into_custom_logo', 10, 2 );

/**
 * Filters the custom logo HTML to inject NHS-specific span markup based on header layout.
 *
 * This function appends the `nhsuk-header__service-logo` class to the <a> tag generated by `get_custom_logo()`,
 * and conditionally injects span elements inside the tag depending on the header layout.
 *
 * - For transactional layout: adds a single `<span class="nhsuk-header__service-name">` with dynamic text.
 * - For non-transactional layout: adds multiple spans including organisation name, split lines, and descriptor.
 *
 * The layout is determined via `get_theme_mod( 'header_layout' )`, which should return either `'transactional'`
 * or `'default'`. You can customize this logic to use other theme options or context checks.
 *
 * @param string $html     The original custom logo HTML output.
 *
 * @return string Modified custom logo HTML with injected span markup and appended class.
 *
 * @hook get_custom_logo
 */
function inject_logo_markup_into_custom_logo( $html ) {
	if ( strpos( $html, '<a ' ) !== false ) {
		...
		// Append class to <a> tag.
		$html = preg_replace(
			'/<a\s([^>]*class="[^"]*)"/',
			'<a $1 nhsuk-header__service-logo"',
			$html
		);

		if ( $header_layout === 'transactional' ) {
			$span_html = '<span class="nhsuk-header__service-name">' . esc_html( $logo_line_1 ) . '</span>';
		} else {
			// Start building span block.
			$span_html  = '<span class="nhsuk-header__organisation-name">';
			$span_html .= esc_html( array_shift( $logo_lines ) );

			foreach ( $logo_lines as $logo_line ) {
				$span_html .= '<span class="nhsuk-header__organisation-name-split">' . esc_html( $logo_line ) . '</span>';
			}

			$span_html .= '</span>';
			$span_html .= '<span class="nhsuk-header__organisation-name-descriptor">' . esc_html( $logo_line_2 ) . '</span>';
		}
		// Inject before closing </a>.
		$html = str_replace( '</a>', $span_html . '</a>', $html );
	}
	return $html;
}
